    ...
    <nav class="main_nav">
        {block name='frontend_index_navigation_categories_navigation_list'}
            <ul>
                {strip}
                    {block name='frontend_index_navigation_categories_top_home'}
                        <li class="hassubs {if $sCategoryCurrent == $sCategoryStart && $Controller == 'index'} active{/if}" role="menuitem">
                            {block name='frontend_index_navigation_categories_top_link_home'}
                                <a href="{url controller='index'}" title="{s name='IndexLinkHome' namespace="frontend/index/categories_top"}{/s}" itemprop="url">{s name='IndexLinkHome' namespace="frontend/index/categories_top"}{/s}</a>
                            {/block}
                        </li>
                    {/block}
                        {block name='frontend_index_navigation_categories_top_before'}{/block}
                        {foreach $sMainCategories as $sCategory}
                            {block name='frontend_index_navigation_categories_top_entry'}
                                {if !$sCategory.hideTop}
                                    <li role="menuitem">
                                        {block name='frontend_index_navigation_categories_top_link'}
                                            <a href="{$sCategory.link}" title="{$sCategory.description}" itemprop="url"{if $sCategory.external && $sCategory.externalTarget} target="{$sCategory.externalTarget}"{/if}>
                                                <span itemprop="name">{$sCategory.description}</span>
                                            </a>
                                        {/block}
                                    </li>
